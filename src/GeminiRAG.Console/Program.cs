           ...
           if (geminiSettings.ApiKey == "YOUR_API_KEY_HERE")
            ...
            if (stores.Count > 0)
            {
                ...
                if (int.TryParse(choice, out int storeIndex) && storeIndex > 0 && storeIndex <= stores.Count)
                {
                    ...
                    ConsoleUI.WriteSuccess($"✓ Using existing store: {selectedStoreName}");
                }
            ...
            var existingFiles = await fileSearchService.ListFilesAsync(selectedStoreName!);
            ...
            if (existingFiles.Count > 0)
            {
                ...
                else if (userChoice == "2")
                {
                    ...
                    {
                        await fileSearchService.UploadPdfAsync(selectedStoreName!, pdfPath);
                    }
                }
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        Question = question,
                        FileSearchStoreName = selectedStoreName!
                    };
            ...
            {
                await fileSearchService.DeleteStoreAsync(selectedStoreName!);
            }
